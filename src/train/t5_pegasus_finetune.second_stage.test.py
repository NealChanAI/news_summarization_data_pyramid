...
string_classes = (str,)
int_classes = (int,)
from transformers import MT5ForConditionalGeneration, BertTokenizer, AutoConfig
...
def create_data(data, tokenizer, max_len=512, term='train'):
    ...
    for title, content in data:
        ...
        # 创建关键词注意力掩码
        keyword_mask = [0] * len(text_ids)  # 初始为0
        for i, token_id in enumerate(text_ids):
            token = tokenizer.decode([token_id])  # 将token id解码为文本
            if token in KEYWORDS:
                keyword_mask[i] = 1  # 如果token是关键词，则设置为1
        ...
        if term == 'train':
            ...
            features = {'input_ids': text_ids,
                        ...
                        'keyword_mask': keyword_mask  # 添加关键词注意力掩码
                        }
...
class MT5ForConditionalGenerationWithKeywordAttention(MT5ForConditionalGeneration):
    def __init__(self, config):
        super().__init__(config)
        self.keyword_attention = nn.Linear(config.hidden_size, 1)  # 简单的线性层
    ...
    def forward(
        ...
        return_dict=None,
        keyword_mask=None,  # 添加关键词掩码
    ):
        # 使用原始的T5模型获取encoder的输出
        ...
        if keyword_mask is not None:
            keyword_mask = keyword_mask.to(encoder_outputs[0].dtype)  # 确保数据类型一致
            keyword_attention_scores = self.keyword_attention(encoder_outputs[0]).squeeze(-1)  # 计算关键词注意力得分
            keyword_attention_weights = torch.sigmoid(keyword_attention_scores) * keyword_mask  # 应用sigmoid函数和关键词掩码
            encoder_outputs = (encoder_outputs[0] * (1 + keyword_attention_weights.unsqueeze(-1)),) + encoder_outputs[1:]  # 将注意力权重应用到encoder输出上
...
def train_model(model, adam, train_data, dev_data, tokenizer, device, args):
    ...
    for epoch in range(args.num_epoch):
        ...
        for i, cur in enumerate(tqdm(train_data, desc='Epoch {}:'.format(epoch))):
            ...
            keyword_mask = cur.get('keyword_mask')  # 获取关键词掩码
            prob = model(input_ids=cur['input_ids'], attention_mask=cur['attention_mask'],
                         decoder_input_ids=cur['decoder_input_ids'], decoder_attention_mask=cur['decoder_attention_mask'],
                         keyword_mask=keyword_mask)[0]  # 将关键词掩码传递给模型
        ...
        for feature in tqdm(dev_data):
            ...
            keyword_mask = content.pop('keyword_mask', None)  # 获取关键词掩码并从content中移除
...
if __name__ == '__main__':
    ...
    model = MT5ForConditionalGenerationWithKeywordAttention.from_pretrained(args.pretrain_model, config=config).to(device)
