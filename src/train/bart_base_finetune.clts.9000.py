...
from transformers import BertTokenizer, BartForConditionalGeneration, TrainingArguments, Trainer, DataCollatorForSeq2Seq
...
SAVED_MODEL_PATH = osp.join(ROOT_DIR, 'model', 'chinese_bart_base_finetune_clts_9000')
TRAIN_INFO_PATH = osp.join(ROOT_DIR, 'model', 'chinese_bart_base_train_info')


# 加载tokenizer和模型
tokenizer = BertTokenizer.from_pretrained(PRETRAIN_MODEL_PATH)  # 使用 BertTokenizer
model = BartForConditionalGeneration.from_pretrained(PRETRAIN_MODEL_PATH)
...
if __name__ == '__main__':
    TRAIN_FILE_PATH = 'train_formatted_clts_data.9000.txt'  # 训练数据集
    VALIDATE_FILE_PATH = 'train_formatted_clts_data.1000.txt'  # 验证数据集
    training_workflow()
