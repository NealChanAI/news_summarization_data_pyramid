...
def train_model(model, adam, train_data, dev_data, tokenizer, device, args):
    ...
    for epoch in range(args.num_epoch):
        ...
        if rouge_l > best:
            best = rouge_l
            if args.data_parallel and torch.cuda.is_available():
                torch.save(model.module, os.path.join(model_save_path, args.stage + '_' + args.version + '_' + 'lcsts_stage2'))
            else:
                torch.save(model, os.path.join(model_save_path, args.stage + '_' + args.version + '_' + 'lcsts_stage2'))
...
if __name__ == '__main__':
    ...
    LOG_FILE = osp.join(LOG_DIR, args.model_specific_dir, f'{args.version}.{args.stage}.train.{current_time}.lcsts_stage2.log')
